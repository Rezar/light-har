...
final class LightHarTransformerTests: XCTestCase {

    var model: MLModel?
    var modelOutput: String = "linear_10"
    var ttl: Double = 10.0
    ...
    func testTransformer() {
        ...
        measurePerformance {
            try self.modelPrediction(ttl: self.ttl) { inputArray in
                TransformerInput(src_1: inputArray)
            }
        }
    }
    
    func testTransformerInt8LSQuantization() {
        model = try? TransformerInt8LSQuantization().model
        measurePerformance {
            try self.modelPrediction(ttl: self.ttl) { inputArray in
                TransformerInt8LSQuantizationInput(src_1: inputArray)
            }
        }
    }

    func testTransformerMagnitudePruner() {
        model = try? TransformerMagnitudePruner().model
        measurePerformance {
            try self.modelPrediction(ttl: self.ttl) { inputArray in
                TransformerMagnitudePrunerInput(src_1: inputArray)
            }
        }
    }

    func testTransformerPalettizationLUT() {
        model = try? TransformerPalettizationLUT().model
        measurePerformance {
            try self.modelPrediction(ttl: self.ttl) { inputArray in
                TransformerPalettizationLUTInput(src_1: inputArray)
            }
        }
    }

    func testTransformerThresholdPruner() {
        model = try? TransformerThresholdPruner().model
        measurePerformance {
            try self.modelPrediction(ttl: self.ttl) { inputArray in
                TransformerThresholdPrunerInput(src_1: inputArray)
            }
        }
    }
}
