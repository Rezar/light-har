...
final class LightHarLSTMTests: XCTestCase {

    var model: MLModel?
    var modelOutput: String = "linear_0"
    var ttl: Double = 10.0
    ...
    func testLSTM() {
        ...
        measurePerformance {
            try self.modelPrediction(ttl: self.ttl) { inputArray in
                LSTMInput(x: inputArray)
            }
        }
    }
    
    func testLSTMInt8LSQuantization() {
        model = try? LSTMInt8LSQuantization().model
        measurePerformance {
            try self.modelPrediction(ttl: self.ttl) { inputArray in
                LSTMInt8LSQuantizationInput(x: inputArray)
            }
        }
    }

    func testLSTMMagnitudePruner() {
        model = try? LSTMMagnitudePruner().model
        measurePerformance {
            try self.modelPrediction(ttl: self.ttl) { inputArray in
                LSTMMagnitudePrunerInput(x: inputArray)
            }
        }
    }

    func testLSTMPalettizationLUT() {
        model = try? LSTMPalettizationLUT().model
        measurePerformance {
            try self.modelPrediction(ttl: self.ttl) { inputArray in
                LSTMPalettizationLUTInput(x: inputArray)
            }
        }
    }

    func testLSTMThresholdPruner() {
        model = try? LSTMThresholdPruner().model
        measurePerformance {
            try self.modelPrediction(ttl: self.ttl) { inputArray in
                LSTMThresholdPrunerInput(x: inputArray)
            }
        }
    }
}
