...
final class LightHarMLPTests: XCTestCase {

    var model: MLModel?
    var modelOutput: String = "linear_2"
    var ttl: Double = 10.0
    ...
    func testMLP() {
        ...
        measurePerformance {
            try self.modelPrediction(ttl: self.ttl) { inputArray in
                MLPInput(x: inputArray)
            }
        }
    }
    
    func testMLPInt8LSQuantization() {
        model = try? MLPInt8LSQuantization().model
        measurePerformance {
            try self.modelPrediction(ttl: self.ttl) { inputArray in
                MLPInt8LSQuantizationInput(x: inputArray)
            }
        }
    }

    func testMLPMagnitudePruner() {
        model = try? MLPMagnitudePruner().model
        measurePerformance {
            try self.modelPrediction(ttl: self.ttl) { inputArray in
                MLPMagnitudePrunerInput(x: inputArray)
            }
        }
    }

    func testMLPPalettizationLUT() {
        model = try? MLPPalettizationLUT().model
        measurePerformance {
            try self.modelPrediction(ttl: self.ttl) { inputArray in
                MLPPalettizationLUTInput(x: inputArray)
            }
        }
    }

    func testMLPThresholdPruner() {
        model = try? MLPThresholdPruner().model
        measurePerformance {
            try self.modelPrediction(ttl: self.ttl) { inputArray in
                MLPThresholdPrunerInput(x: inputArray)
            }
        }
    }
}
